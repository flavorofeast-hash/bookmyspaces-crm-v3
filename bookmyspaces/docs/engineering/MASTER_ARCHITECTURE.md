...
- **New integrations register, not fork.** Facebook Messenger and Instagram DM (originally scoped as Phase 4; capture built in an earlier session via `dm-capture-service.ts`, AI-response completed 2026-08-01 — Version 2.0, Omnichannel Communication Platform — via `dm-responder.ts`/`dm-send.ts`) each added one adapter/service module plus reuse of the existing `channels` row and `unified_conversations`/`unified_messages` tables — not a parallel inbox, parallel identity system, or parallel timeline, and not a channel-specific AI prompt (both reuse `chatWithAI()`/`SYSTEM_PROMPT` verbatim). This is the same reuse discipline `MASTER_DATABASE.md` applies to tables, applied to integration code. See `docs/sprints/2026-08-01_omnichannel-dm-ai-response.md`.

...
- **Grade: Yellow — known drift history, but the code's `select('*')` pattern limits blast radius to "missing metadata," not "hard failure."**
...
| **Critical** | Migration **027** — new finding, hard-failure dependency of the entire Site Visit → Proposal pipeline validated last pass, never previously checked. Migrations 016/017/024's documented silent-failure bugs (carried forward, unchanged). Migrations 012/013 (Reservations, entire module non-functional if genuinely absent). ENG-004 (reservation pricing-zeroing, unresolved). |
...
**READY WITH MINOR ISSUES — unchanged from last pass's `RC2_READINESS_REPORT.md`, with one new required action before shipping.**

Application-layer logic remains fully validated (last pass). This pass adds no new code-level risk, but surfaces one previously-unknown, high-severity database-state unknown (migration 027) that directly threatens the pipeline just validated. **Recommended sequencing before this release ships:**

1. Run `scripts/verify-migrations-026-027.sql` against production — five minutes, decisive, resolves this report's single Critical new finding.
2. If 027 is missing, apply `supabase/migrations/027_site_visit_fields.sql` directly (additive, idempotent, safe) before any AI chat conversation or `/visits/new` submission is trusted in production.
3. Separately, and not blocking this release per se: run the existing one-shot query in `RC1_DEPLOYMENT_READINESS.md` §1 and `PRODUCTION_MIGRATION_STATE_VERIFICATION.md` §2 to finally convert ENG-001–004 from "presumed"/"unverified" to fact — carried forward, unresolved, same recommendation as every prior audit in this project.

No code was changed to produce this report, other than the two new read-only, information_schema-only verification SQL files, which run nothing against application data and modify nothing.
